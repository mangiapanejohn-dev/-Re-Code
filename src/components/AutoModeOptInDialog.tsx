...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets RE CODE handle permission prompts automatically — RE CODE checks each tool call for risky actions and prompt injection before executing. Actions RE CODE identifies as safe are executed, while actions RE CODE identifies as risky are blocked and RE CODE may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. RE CODE can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
